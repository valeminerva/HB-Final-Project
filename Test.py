...
def find_verb_1st_conj_ind_pres(splitted_sentence):
	...
	for word in splitted_sentence:
		if word[-1] in des_ind_pres_1_conj.values() and word[-2:] != "eo" and word[-2:] != "io":
			ending_def = des_ind_pres_1_conj.keys()[des_ind_pres_1_conj.values().index(word[-1])]
			word_ending = word[-1]
			print "The verb is %s. It is a %s, 1st conj." % (word, ending_def)
			return True
		elif word[-2:] in des_ind_pres_1_conj.values():
			ending_def = des_ind_pres_1_conj.keys()[des_ind_pres_1_conj.values().index(word[-2:])]
			word_ending = word[-2:]
			print "The verb is %s. It is a %s, 1st conj." % (word, ending_def)
			return True
		elif word[-3:] in des_ind_pres_1_conj.values():
			ending_def = des_ind_pres_1_conj.keys()[des_ind_pres_1_conj.values().index(word[-3:])] 
			word_ending = word[-3:]
			print "The verb is %s. It is a %s, 1st conj." % (word, ending_def)
			return True
		elif word[-4:] in des_ind_pres_1_conj.values():
			ending_def = des_ind_pres_1_conj.keys()[des_ind_pres_1_conj.values().index(word[-4:])]
			word_ending = word[-4:]
			print "The verb is %s. It is a %s, 1st conj." % (word, ending_def)
			return True
...
def find_verb_2nd_conj_ind_pres(splitted_sentence):
	...
	for word in splitted_sentence:
		if 	word[-2:] in des_ind_pres_2_conj.values():
			ending_def = des_ind_pres_2_conj.keys()[des_ind_pres_2_conj.values().index(word[-2:])]
			word_ending = word[-2:]
			print "The verb is %s. It is a %s." % (word, ending_def)
			return True
		elif word[-3:] in des_ind_pres_2_conj.values():
			ending_def = des_ind_pres_2_conj.keys()[des_ind_pres_2_conj.values().index(word[-3:])]
			word_ending = word[-3:]
			print "The verb is %s. It is a %s." % (word, ending_def)
			return True
		elif word[-4:] in des_ind_pres_2_conj.values():
			ending_def = des_ind_pres_2_conj.keys()[des_ind_pres_2_conj.values().index(word[-4:])]
			word_ending = word[-4:]
			print "The verb is %s. It is a %s." % (word, ending_def)
			return True
...
def find_verb_4_conj_ind_pres(splitted_sentence):
	...
	for word in splitted_sentence:
		if 	word[-2:] in des_ind_pres_4_conj.values():
			ending_def = des_ind_pres_4_conj.keys()[des_ind_pres_4_conj.values().index(word[-2:])]
			word_ending = word[-2:]
			print "The verb is %s. It is a %s, 4th conj" % (word, ending_def)
			return True
		elif word[-3:] in des_ind_pres_4_conj.values():
			ending_def = des_ind_pres_4th_conj.keys()[des_ind_pres_4_conj.values().index(word[-3:])]
			word_ending = word[-3:]
			print "The verb is %s. It is a %s." % (word, ending_def)
			return True
		elif word[-4:] in des_ind_pres_4_conj.values():
			ending_def = des_ind_pres_4th_conj.keys()[des_ind_pres_4_conj.values().index(word[-4:])]
			word_ending = word[-4:]
			print "The verb is %s. It is a %s." % (word, ending_def)
			return True
