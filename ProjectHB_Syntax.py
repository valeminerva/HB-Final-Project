...
des_subst_nominat = {"1a decl":"a", "2a decl":"us", "2a decl -er":"er"}
des_subst_accusat = {"1a decl":"am", "2a decl":"um", "2a decl -er": "em"}
...
def find_subject():
	...
	splitted_sentence = sentence()
	for word in splitted_sentence:
			if word[-1] == "a" or word[-2:] in des_subst_nominat.values():
...
def analyze_word():
	...
	if word_to_analyze[-1] == "a" or word_to_analyze[-2:] in des_subst_nominat.values():
		print "%s is a noun. Nom. Sg." % (word_to_analyze)
	elif word_to_analyze[-2:] in des_subst_accusat.values():
